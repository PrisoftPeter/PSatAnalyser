package gla.server.kernel.knowledge.worlds;
...
public class K42a extends World implements Serializable {
	...
	public K42a(Agent self, Agent agent1, Agent agent2, Attribute attribute){
		this.self = self;
		this.agent1 = agent1;
		this.agent2 = agent2;
		...
		super.type = "K42a";
		super.htmlType = "<i>k</i><sub>42a</sub>";
	...
	public String getGenericMeaning(String genericSelf, String genericAgent1, String genericAgent2) {
		return genericSelf+" beliefs that "+genericAgent2+" knows that "+genericAgent1+" knows "+attribute.toHtmlString();
	}
	
	@Override
	public String getMeaning() {
		return self.getAgentName()+" beliefs that "+agent2.getAgentName()+" knows that "+agent1.getAgentName()+" knows "+attribute.toHtmlString();
	}
