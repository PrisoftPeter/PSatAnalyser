...
public class Memory implements Serializable{	
	...
	public Memory(Agent self,String subjectName){
		this.self = self;
		this.subjectName = subjectName;
		otheragents = new Agent[0];
		setOtherAgents(PSatAPI.instance);
		createMemoryStorePath(PSatAPI.instance.sessionid);
		createAssertionsStorePath(PSatAPI.instance.sessionid);
	}
	...
	public boolean substitute(World oldworld, World newworld, String processName){

		if(PSatAPI.instance.learningMaxSubs){
		...
		if(removed){
			...
			if(PSatAPI.instance.log_knowledge_transformation){	
				...
				Display.updateLogPage(info0, false);
			}			
			return true;
		}
		if(PSatAPI.instance.log_knowledge_transformation){
			...
			Display.updateLogPage(info1, false);			
		}		
		return false;
	}
		...
		try {
			...
			if(folder2.isDirectory()){
				for (final File fileEntry : folder2.listFiles()) {
					...
					boolean isComposite = MemoryFactory.isCompositeWorld(w);
