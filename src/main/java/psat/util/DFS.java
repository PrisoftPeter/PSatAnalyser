...
public class DFS<E> {
    ...
    public void Initialize(){
        if(sourceNode != null){
            ...
            this.findAllPaths_(g, visited, paths, sourceNode);	
        }        
    }
    
    @SuppressWarnings({ "rawtypes", "unchecked" })
	private void findAllPaths_(Graph graph, List<E> visited, List<ArrayList<E>> paths, Object currentNode) {        

	    if(paths.size() >= PSatAPI.instance.max_no_analysis_paths){
	    	return;
	    }
	        ...
	        for (E node : nodes) {
	            ...
	            temp.addAll(visited);
	            temp.add(node);
	            if(temp.size() <= this.maxPathSize) findAllPaths_(graph, temp, paths, node);
	        }
	    }
	}
